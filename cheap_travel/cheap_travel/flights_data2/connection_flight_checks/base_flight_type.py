...
class BaseFlightType(object):
    ...
    def _get_next_indexes(self, cookie, response1, response2):
        ...
        if first_jump_price <= second_jump_price and not update_second:
            first_index += 1
            if second_base_index < MAX_FLIGHTS_PER_TRIP -1 and second_base_index < len(response2.trips) -1:
                second_base_index += 1
                second_index = second_base_index
        else:
            second_index += 1
            if first_base_index < MAX_FLIGHTS_PER_TRIP -1 and first_base_index < len(response1.trips) -1:
                first_base_index += 1
